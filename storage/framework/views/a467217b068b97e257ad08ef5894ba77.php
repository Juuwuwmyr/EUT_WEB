...
<div id="cancelModal" style="display:none;position:fixed;bottom:0;left:0;right:0;z-index:1001;background:linear-gradient(145deg,#1a0a0a,#120808);border:1px solid rgba(239,68,68,.25);border-radius:24px 24px 0 0;padding:24px 20px 40px;max-width:540px;margin:0 auto;">
    ...
    <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:18px;" id="cancelReasons">
        ...
        <label style="display:flex;align-items:center;gap:12px;padding:12px 14px;border-radius:12px;border:1.5px solid rgba(255,255,255,.07);cursor:pointer;transition:all .2s;">
            <input type="radio" name="cancelReason" value="<?php echo e($reason); ?>" style="accent-color:#ef4444;width:16px;height:16px;" onchange="document.querySelectorAll('#cancelReasons label').forEach(l=>l.style.borderColor='rgba(255,255,255,.07)');this.closest('label').style.borderColor='rgba(239,68,68,.5)';">
...
function buildOrderCard(o) {
    ...
    const imgs = o.items.slice(0,3).map(i=>
        `<img src="${escHtml(i.image)}" class="ocard-img" alt="${escHtml(i.name)}" onerror="this.src='<?php echo e(asset('images/hero-burger.jpg')); ?>'">`)
        .join('');
...
function emptyState(icon, title, sub, showBtn) {
    return `<div class="empty-state">
        ...
        ${showBtn?`<a href="<?php echo e(route('shop.home')); ?>" class="btn-primary">Browse Menu</a>`:''}
    </div>`;
}
...
function buildDetailBody(o) {
    ...
    const itemsHtml = `
        <div class="sheet-section" style="padding-bottom:14px;">
            ...
            ${o.items.map(i=>`
            <div class="sitem">
                <img src="${escHtml(i.image)}" class="sitem-img" alt="${escHtml(i.name)}" onerror="this.src='<?php echo e(asset('images/hero-burger.jpg')); ?>'">
    ...
    const reorderHtml = (o.status==='delivered'||o.status==='cancelled') ? `
        <div style="padding:14px 18px 32px;display:flex;justify-content:center;">
            <a href="<?php echo e(route('shop.home')); ?>" class="btn-reorder">🔁 Order Again</a>
...
async function loadAllOrders() {
    try {
        const res = await fetch('/orders', {headers:{'Accept':'application/json','X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'}});
...
async function submitCancel(){
    ...
    try{
        const r=await fetch(`/orders/${currentCancelOrderId}/cancel`,{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json','X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'},body:JSON.stringify({reason:sel.value})});
...
async function initOrderMap(order) {
    ...
    // Use saved delivery coords — set at checkout time. Never use live browser GPS here
    // so the destination pin never jumps around on each poll.
    ...
    if (!isOnWay) {
        // Order not yet with rider — just show restaurant + destination
    ...
    if (isOut) {
        // Out for delivery → route from rider to customer
        ...
        if (route) {
            ...
            if (etaEl) {
                const mins = Math.max(1, Math.round(route.length / 30));
                etaEl.textContent = '~' + mins + ' min away';
            }
        } else {
        ...
        // rider_assigned → show planned route restaurant → customer
