...
<div class="page-header" style="display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:1rem;">
    <div style="display:flex;align-items:center;gap:.75rem;">
        ...
        <div><h1 style="margin:0 0 .15rem;">Menu Items</h1><p style="margin:0;"><?php echo e($items->count()); ?> items shown.</p></div>
    </div>
    <div style="display:flex;gap:.625rem;">
        <a href="<?php echo e(route('admin.menu-items', array_merge(request()->query(), ['archived' => request('archived') ? null : '1']))); ?>"
           class="btn-ghost" style="display:inline-flex;align-items:center;gap:.4rem;font-size:.8rem;">
            <i data-lucide="<?php echo e(request('archived') ? 'eye' : 'archive'); ?>" style="width:.85rem;height:.85rem;stroke-width:2;"></i>
            <?php echo e(request('archived') ? 'View Active' : 'View Archived'); ?>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
    <?php $__currentLoopData = $summaryCards; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <a href="<?php echo e(route('admin.menu-items',['category'=>$sc['slug']])); ?>" class="stat-card"
       style="text-decoration:none;position:relative;overflow:hidden;border-color:<?php echo e($sc['color']); ?>22;
              <?php echo e(request('category')===$sc['slug'] ? 'border-color:'.$sc['color'].'55;box-shadow:0 0 0 3px '.$sc['color'].'18;' : ''); ?>">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:.875rem;">
            <div style="width:3rem;height:3rem;border-radius:.875rem;background:<?php echo e($sc['bg']); ?>;display:flex;align-items:center;justify-content:center;">
                <i data-lucide="<?php echo e($sc['icon']); ?>" style="width:1.4rem;height:1.4rem;color:<?php echo e($sc['color']); ?>;stroke-width:2;"></i>
            </div>
            <span style="font-size:2.5rem;font-weight:900;color:<?php echo e($sc['color']); ?>;line-height:1;"><?php echo e($sc['count']); ?></span>
        </div>
        <h3 style="font-size:1rem;font-weight:700;color:var(--text-strong);margin:0 0 .25rem;"><?php echo e($sc['label']); ?></h3>
        <span style="font-size:.72rem;font-weight:600;color:<?php echo e($sc['color']); ?>;display:inline-flex;align-items:center;gap:.3rem;">
            Browse <i data-lucide="arrow-right" style="width:.7rem;height:.7rem;stroke-width:2.5;"></i>
        </span>
        <div style="position:absolute;bottom:-1.5rem;right:-1.5rem;width:5.5rem;height:5.5rem;border-radius:50%;background:<?php echo e($sc['bg']); ?>;filter:blur(18px);pointer-events:none;"></div>
    </a>
...
<div class="section-card">
    <form method="GET" action="<?php echo e(route('admin.menu-items')); ?>" class="filter-bar">
        <div style="position:relative;max-width:240px;width:100%;">
            ...
            <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Search items…" class="admin-input" style="padding-left:2.1rem;">
        </div>
        ...
        <?php if(request('search') || request('category')): ?>
            <a href="<?php echo e(route('admin.menu-items')); ?>" class="btn-ghost" style="display:inline-flex;align-items:center;gap:.35rem;">
    ...
    <div style="padding:.75rem 1.25rem;border-bottom:1px solid var(--border-divider);display:flex;gap:.5rem;flex-wrap:wrap;align-items:center;">
        <a href="<?php echo e(route('admin.menu-items')); ?>" style="display:inline-flex;align-items:center;gap:.3rem;padding:.3rem .875rem;border-radius:9999px;font-size:.75rem;font-weight:600;text-decoration:none;transition:all .2s;<?php echo e(!request('category') ? 'background:var(--accent);color:#fff;' : 'background:var(--border-card);color:var(--text-muted);'); ?>">
            <i data-lucide="layers" style="width:.75rem;height:.75rem;stroke-width:2.5;"></i> All
        </a>
        <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <a href="<?php echo e(route('admin.menu-items',['category'=>$cat->slug])); ?>" style="display:inline-flex;align-items:center;gap:.3rem;padding:.3rem .875rem;border-radius:9999px;font-size:.75rem;font-weight:600;text-decoration:none;transition:all .2s;<?php echo e(request('category')===$cat->slug ? 'background:'.$cat->color.';color:#fff;' : 'background:var(--border-card);color:var(--text-muted);'); ?>">
            <i data-lucide="<?php echo e($cat->icon); ?>" style="width:.75rem;height:.75rem;stroke-width:2.5;"></i> <?php echo e($cat->name); ?>
    ...
    <table class="admin-table">
        ...
        <tbody>
            ...
            <tr style="<?php echo e($item->is_archived ? 'opacity:.55;' : ''); ?>">
                <td>
                    <div style="display:flex;align-items:center;gap:.75rem;">
                        <img src="<?php echo e(asset($item->image)); ?>" alt="<?php echo e($item->name); ?>" style="width:3rem;height:3rem;border-radius:.75rem;object-fit:cover;border:1px solid var(--border-card);flex-shrink:0;">
                        <div>
                            <span style="font-weight:600;color:var(--text-strong);display:block;"><?php echo e($item->name); ?></span>
                            <span style="font-size:.72rem;color:var(--text-muted);"><?php echo e(Str::limit($item->description,45)); ?></span>
                        </div>
                    </div>
                </td>
                <td>
                    <span class="badge" style="background:<?php echo e($item->category->color); ?>18;color:<?php echo e($item->category->color); ?>;display:inline-flex;align-items:center;gap:.3rem;">
                        <i data-lucide="<?php echo e($item->category->icon); ?>" style="width:.65rem;height:.65rem;stroke-width:2.5;"></i>
                        <?php echo e($item->category->name); ?>

                    </span>
                </td>
                <td style="font-weight:700;color:<?php echo e($item->category->color); ?>;">₱<?php echo e(number_format($item->price,2)); ?></td>
                <td>
                    <?php $flavors = $item->modifierGroups->where('type','flavor'); ?>
                    <?php if($flavors->count()): ?>
                        <span class="badge badge-preparing" style="display:inline-flex;align-items:center;gap:.25rem;">
                            <i data-lucide="droplets" style="width:.65rem;height:.65rem;stroke-width:2;"></i> <?php echo e($flavors->count()); ?> group<?php echo e($flavors->count()>1?'s':''); ?>
                ...
                <td>
                    <?php $mods = $item->modifierGroups->where('type','modifier'); ?>
                    <?php if($mods->count()): ?>
                        <span class="badge badge-out" style="display:inline-flex;align-items:center;gap:.25rem;">
                            <i data-lucide="sliders-horizontal" style="width:.65rem;height:.65rem;stroke-width:2;"></i> <?php echo e($mods->count()); ?> group<?php echo e($mods->count()>1?'s':''); ?>
                ...
                <td>
                    <?php $addons = $item->modifierGroups->where('type','addon'); ?>
                    <?php if($addons->count()): ?>
                        <span class="badge" style="background:rgba(245,158,11,.1);color:#d97706;display:inline-flex;align-items:center;gap:.25rem;">
                            <i data-lucide="package-plus" style="width:.65rem;height:.65rem;stroke-width:2;"></i> <?php echo e($addons->count()); ?> add-on<?php echo e($addons->count()>1?'s':''); ?>
                ...
                <td>
                    <div style="display:flex;align-items:center;justify-content:center;gap:.375rem;">
                        <button onclick='openEditItemModal(<?php echo json_encode($item->load("modifierGroups.options"), 15, 512) ?>, <?php echo e($categories->toJson()); ?>)' class="btn-icon-edit" title="Edit">
                        ...
                        <form method="POST" action="<?php echo e(route('admin.menu-items.archive',$item)); ?>">
                            ...
                            <button type="submit" class="<?php echo e($item->is_archived ? 'btn-icon-restore' : 'btn-icon-archive'); ?>" title="<?php echo e($item->is_archived?'Restore':'Archive'); ?>"
                                ...
                                <i data-lucide="<?php echo e($item->is_archived?'archive-restore':'archive'); ?>" style="width:.8rem;height:.8rem;stroke-width:2.5;"></i>
                            </button>
                        </form>
                        <form method="POST" action="<?php echo e(route('admin.menu-items.delete',$item)); ?>">
                            <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                            <button type="submit" class="btn-icon-delete" title="Delete"
                                    onclick="return confirm('Permanently delete <?php echo e(addslashes($item->name)); ?>?')">
...
function openAddItemModal() {
    ...
    document.getElementById('itemModalForm').action          = '<?php echo e(route("admin.menu-items.store")); ?>';
...
function openEditItemModal(item, categories) {
    ...
    document.getElementById('itemModalForm').action          = '<?php echo e(url("admin/menu-items")); ?>/' + item.id;
...
<?php echo $__env->make('admin.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\patri\Desktop\EUT_WEB\resources\views/admin/menu-items.blade.php ENDPATH**/ ?>