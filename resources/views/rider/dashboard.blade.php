...
<head>
    ...
    <title>Rider Dashboard Äî EUT</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Inter', sans-serif; }
        ...
        /* îÄîÄ DELIVERY CONFIRM SHEET îÄîÄ */
        .sheet-backdrop {
        ...
        /* îÄîÄ STEP VIEWS inside sheet îÄîÄ */
    ...
    <div id="view-active">
        ...
        <div style="background:linear-gradient(145deg,#12131f,#0e0f1a);border:1px solid rgba(139,92,246,.3);border-radius:18px;overflow:hidden;margin-bottom:12px;">
            ...
            <div style="position:relative;">
                ...
                <!-- Re-center button ó appears after rider pans away -->
        ...
        @foreach($active as $order)
            @if($loop->first && $order->status === 'rider_assigned')
                <p class="section-label" style="margin-top:0;">?? Assigned ó Head to Restaurant</p>
            @elseif($order->status === 'rider_assigned')
                <p class="section-label" style="margin-top:20px;">?? Assigned ó Head to Restaurant</p>
                ...
                <div class="oc-body">
                    <div class="oc-address-row">
                        ...
                        <div>
                            ...
                            <p class="oc-addr-val">
                                {{ $order->status === 'out_for_delivery' ? '? Picked up at ' . $order->picked_up_at?->format('g:i A') : 'E.U.T Snack House ó Metro Naujan' }}
                            </p>
                        </div>
                    </div>
                    <div class="oc-address-row">
    ...
    <div style="background:linear-gradient(135deg,#1a0e00,#100f1a);border:1px solid rgba(245,158,11,.2);border-radius:20px;padding:20px 18px;margin-bottom:14px;position:relative;overflow:hidden;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
            ...
            <div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);border-radius:12px;padding:12px 14px;">
                ...
                <p style="font-size:14px;font-weight:700;color:#fff;margin:0;">{{ $rider->phone ?? 'ó' }}</p>
            </div>
            <div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);border-radius:12px;padding:12px 14px;">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
            <div>
                <p style="font-size:14px;font-weight:700;color:#fff;margin:0 0 3px;" id="profileStatusLabel">
                    {{ $rider->is_available ? 'Online ó Ready for orders' : 'Offline ó Not accepting orders' }}
                </p>
...
<nav class="bottom-nav">
    <div class="bottom-nav-inner">
        <a href="#" class="bnav-item active" onclick="switchTab('active');return false;">
            <svg width="22" height="22" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01"/></svg>
            Orders
        </a>
        <a href="#" class="bnav-item" onclick="switchTab('history');return false;">
            <svg width="22" height="22" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M3 3v5h5"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M12 7v5l4 2"/></svg>
            History
        </a>
        <a href="#" class="bnav-item" onclick="switchTab('earnings');return false;">
            <svg width="22" height="22" fill="none" stroke="currentColor" viewBox="0 0 24 24"><line x1="12" y1="1" x2="12" y2="23" stroke-width="1.75" stroke-linecap="round"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
            Earnings
        </a>
    ...
    <div class="sheet">
        ...
        <div class="sheet-step" id="sheet-step-2-present">
            <div style="text-align:center;padding:8px 0 20px;">
                <div style="font-size:52px;margin-bottom:12px;">§ù</div>
            ...
            <button class="btn-sheet-back" onclick="sheetStep(1)">Üê Back</button>
        </div>
...
<script>
/* îÄîÄ Online/Offline Toggle îÄîÄ */
...
function applyOnlineUI(online) {
    ...
    if (label2) label2.textContent = online ? 'Online ó Ready for orders' : 'Offline ó Not accepting orders';
}
...
async function confirmDelivered(type) {
    ...
    if (presBtn)  { presBtn.disabled  = true; presBtn.textContent  = 'SavingÖ'; }
    if (photoBtn) { photoBtn.disabled = true; photoBtn.textContent = 'SavingÖ'; }
...
async function geocodeAddress(rawAddr) {
    ...
    // Strip leading "Name, " prefix ó delivery_address is stored as "Name, street address"
    ...
    for (const q of attempts) {
        try {
            ...
            if (data && data.length) {
                ...
                // Sanity check ó must be somewhere in Philippines (roughly)
        ...
        // Nominatim rate limit ó small delay between attempts
        await new Promise(r => setTimeout(r, 400));
    }

    // Absolute fallback: use Naujan, Oriental Mindoro area center
    console.warn('Geocode failed for:', rawAddr, 'ó using area fallback');
...
async function initRiderMap() {
    ...
    // Rider marker ó animated pulse with motorbike icon
...
/* -------------------------------------------
   GPS ó single watcher for ping + map update
------------------------------------------- */
...
function retryGps() {
    // Don't hide the gate ó startGpsWatch will hide it on success
    startGpsWatch();
}
...
function startGpsWatch() {
    ...
    _gpsWatchId = navigator.geolocation.watchPosition(
        async pos => {
            ...
            // Auto-recenter ó only if rider hasn't manually panned in the last 8 seconds
            if (riderMapL) {
            ...
            // Refresh OSRM route ó throttled to once per 20 seconds
            ...
            // Ping server ó throttled to once per 10 seconds
...
if (navigator.permissions) {
    navigator.permissions.query({ name: 'geolocation' }).then(result => {
        if (result.state === 'denied') {
            setGpsLabel(false, 'Location denied ó tap to enable');
            showGpsBanner(true);
        } else {
            startGpsWatch(); // 'granted' or 'prompt' ó start watching (will trigger browser prompt if needed)
        }
        result.onchange = () => {
